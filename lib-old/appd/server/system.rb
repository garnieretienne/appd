...
module Appd
  ...
  module Server
    ...
    class System
      ...
      # Upload a local SSH key to the server sysops keys path
      def upload_sysop_key(user, path)
        ssh.send_file path, "/tmp/#{user}.pub"
        ssh.exec! "mkdir --parents #{SYSOPS_KEYS_PATH}", sudo: true
        ssh.exec! "mv /tmp/#{user}.pub #{SYSOPS_KEYS_PATH}", sudo: true
        return "#{SYSOPS_KEYS_PATH}/#{user}.pub"
      end

      # Upload a local SSH key to the server devops keys path
      def upload_devop_key(user, path)
        ssh.send_file path, "/tmp/#{user}.pub"
        ssh.exec! "mkdir --parents #{DEVOPS_KEYS_PATH}", sudo: true
        ssh.exec! "mv /tmp/#{user}.pub #{DEVOPS_KEYS_PATH}", sudo: true
        return "#{DEVOPS_KEYS_PATH}/#{user}.pub"
      end
